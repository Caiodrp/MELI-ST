...
# Substitua 'seu_access_token' pelo seu access_token real
...
def main():
    ...
    for category in categories:
        ...
        blob = bucket.Object(f'{category["name"]}/dados.json')
        blob_trends = bucket.Object(f'{category["name"]}/tendencias.json')  # Blob para as tendências

        # Verifica se o blob já existe
        if blob.key not in [obj.key for obj in bucket.objects.all()]:
            # Se o blob não existir, faz o upload dos dados
            blob.put(Body=json_data)
        else:
            # Se o blob existir, baixa os dados existentes
            existing_data = json.loads(blob.get()['Body'].read().decode())
            ...
            blob.put(Body=json.dumps(existing_data))

        # Faz o upload das tendências
        blob_trends.put(Body=json_trends)
